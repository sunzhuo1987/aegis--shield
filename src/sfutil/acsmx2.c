             ...
             int (*build_tree)(void * id, void **existing_tree) )
{
    ...
    for (i = 0; i < acsm->acsmMaxStates; i++)
    {
        for( mlist=MatchList[i]; mlist!=NULL;
                 mlist=mlist->next )
        {
            if (mlist->id)
            {
                build_tree(mlist->id, &MatchList[i]->rule_option_tree);
            }
            cnt++;
        }
        if (MatchList[i])
...
) 
{
    ...
    */
    ...
    if (build_tree)
    {
        acsmBuildMatchStateTrees2(acsm, build_tree);
    }
#endif
               ...
               void *data ) 
{
    ...
    for( i=0; i<q->inq; i++ )
    {
        ...
        if (mlist)
        {
            if (Match (mlist->id, mlist->rule_option_tree, 0, data) > 0)
            {
                q->inq = 0;
                return 1;
            }
...
acsmFree2 (ACSM_STRUCT2 * acsm) 
{
  ...
  for (i = 0; i < acsm->acsmMaxStates; i++)
  {
      ...
      while (mlist)
      {
          ilist = mlist;
          mlist = mlist->next;
          if (acsm->userfree && ilist->id)
              acsm->userfree(ilist->id);
#ifdef DETECTION_OPTION_TREE
