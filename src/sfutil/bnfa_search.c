             ...
             ) 
{
    ...
    if (build_tree)
    {
        bnfaBuildMatchStateTrees( bnfa, build_tree );
    }
#endif
...
#ifdef DETECTION_OPTION_TREE
                    int (*Match)(bnfa_pattern_t * id, void *tree, int index, void *data), 
#else
...
#else
        /* process all patterns in this group */
        for( mlist=bnfa->q[i]; mlist; mlist=mlist->next )
        {
            patrn = (bnfa_pattern_t*)mlist->data;
        
            /*process a pattern -  case is handled by otn processing */
            res = Match (patrn->userdata, 0, data);
            if ( res > 0 )
            {    /* terminate matching */
                bnfa->inq=0;/* clear the q */
                return 1;
    ...
    */
...
#ifdef DETECTION_OPTION_TREE
                    (int (*)(bnfa_pattern_t * id, void *tree, int index, void *data))
#else
                    (int (*)(bnfa_pattern_t*,int i,void *data))
#endif
    ...
    if( bnfa->bnfaCaseMode == BNFA_PER_PAT_CASE )
    {
        ...
        else
        {
            ret = _bnfa_search_csparse_nfa_q( bnfa, Tx, n, 
#ifdef DETECTION_OPTION_TREE
                (int (*)(void * id, void *tree, int index, void *data))
#else
                (int (*)(void*,int i,void *data))
#endif
                Match, data, sindex, current_state );
        }
    }
