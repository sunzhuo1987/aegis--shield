...
static INLINE void _dir_fill_all(u_int32_t *allocated, u_int32_t index, u_int32_t fill, 
               char length, u_int32_t val, dir_sub_table_t *table) {
...
static INLINE void _dir_fill_less_specific(int index, int fill, 
    ...
    for(; index < fill; index++) {
        ...
        * Therefore, recurse to this next level. */
        ...
        else if(length >= table->lengths[index]) {
            table->entries[index] = val;
            table->lengths[index] = length;
        } 
    }
...
static int _dir_sub_insert(IP ip, int length, int cur_len, GENERIC ptr, 
    ...
    /* Need to traverse to a sub-table */
         ...
         * bits and update the length accordingly. */
         _dir_sub_insert(ip << sub_table->width, length,
...
int sfrt_dir_insert(IP ip, int len, word data_index, 
    ...
    return _dir_sub_insert(ip, len, len, (GENERIC)data_index,
                           0, behavior, root->sub_table, root);
}

/* Traverse sub tables looking for match */
/* Called by dir_lookup and recursively */
static tuple_t _dir_sub_lookup(IP ip, dir_sub_table_t *table) {
    word index = ip >> (ARCH_WIDTH - table->width);

    if( !table->entries[index] || table->lengths[index] ) {
       tuple_t ret;
       ret.index = table->entries[index];
       ret.length = table->lengths[index];

        return ret;
    }

    return _dir_sub_lookup( ip << table->width, 
                            (dir_sub_table_t *)table->entries[index]);
}
...
tuple_t sfrt_dir_lookup(IP ip, void *tbl) {
    ...
    return _dir_sub_lookup(ip, root->sub_table);
}
