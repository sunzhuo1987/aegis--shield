             ...
             int (*build_tree)(void * id, void **existing_tree) )
{
    ...
    if (root->child)
    {
        cnt += KTrieBuildMatchStateNode(root->child, build_tree);
    }
    ...
    if (root->sibling)
    {
        cnt += KTrieBuildMatchStateNode(root->sibling, build_tree);
    }
             ...
             int (*build_tree)(void * id, void **existing_tree) )
{
    ...
    for (i = 0; i < KTRIE_ROOT_NODES; i++)
    {
        ...
        if (root)
        {
            cnt += KTrieBuildMatchStateNode(root, build_tree);
        }
    }
    return cnt;
} 
...
)
{
  ...
  */
  ...
  if (build_tree)
  {
      KTrieBuildMatchStateTrees(ts, build_tree);
  }
#endif
           ...
           void *data ) 
{
    ...
    for( i=0; i<q->inq; i++ )
    {
        ...
        if (pk)
        {
            if (match (pk->id, pk->rule_option_tree, 0, data) > 0)
            {
                q->inq=0; 
                return 1;
            }
   ...
   while( n )
   {
        if( root->edge == *T )
        {
            ...
            if (pk)
            {
                index = (int)(T - bT - pk->n );
                nfound++;
                if (match (pk->id, pk->rule_option_tree, index, data) > 0)
                {
                    return nfound;
                }
            }
...
static 
inline int KTrieSearchQBC( KTRIE_STRUCT * ks, unsigned char * T, int n,
#ifdef DETECTION_OPTION_TREE
                           int(*match)(void * id, void *tree, int index, void *data),
#else
                           int(*match)(void * id, int index, void * data), 
