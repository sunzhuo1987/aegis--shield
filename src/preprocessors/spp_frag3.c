...
#ifdef MPLS
                    if ((ntohs(p->eh->ether_type) == ETHERNET_TYPE_MPLS_MULTICAST) ||
                        (ntohs(p->eh->ether_type) == ETHERNET_TYPE_MPLS_UNICAST))
                    {
                        eh->ether_type = p->eh->ether_type;
                    }
                    else
                    {
 ...
 */
    ...
    if(p->mpls)
    {
        ret = SafeMemcpy(rebuild_ptr, p->mpls, MPLS_HEADER_LEN,
                  rebuild_ptr, rebuild_end);
    	...
    	rebuild_ptr += MPLS_HEADER_LEN;
    	mplsHdrLen = MPLS_HEADER_LEN;
    }
#endif
...
#else /* DONT_TRUNCATE */
        dpkth.caplen = datalink_header_len + mplsHdrLen +
     ...
     */
    ((IPHdr *)defrag_pkt.iph)->ip_len = htons((u_int16_t)(defrag_pkt.pkth->len-datalink_header_len - mplsHdrLen));
