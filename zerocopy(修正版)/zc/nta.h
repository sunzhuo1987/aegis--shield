...
static inline void mbuf_copy_from_linear_data(const struct m_buf  *skb,
 ...
 static inline void mbuf_fill_page_desc(struct m_buf *skb, int i,
                                       struct page *page, int off, int size)
 {
         ...
         frag->size= size;
         mbuf_shinfo(skb)->nr_frags = i + 1;
 }

 static inline struct iphdr *ip_header(const struct m_buf  *skb)
{
         return (struct iphdr *)mbuf_network_header(skb);
}

static inline unsigned int ip_headerlen(const struct m_buf *skb)
{
        return ip_header(skb)->ihl * 4;
}

static inline struct tcphdr *tcp_header(const struct m_buf *skb)
{
        return (struct tcphdr *)mbuf_transport_header(skb);
}

static inline unsigned int tcp_headerlen(const struct m_buf *skb)
{
        return tcp_header(skb)->doff * 4;
}

static inline int mbuf_is_gso(const struct m_buf  *skb)
{
         return mbuf_shinfo(skb)->gso_size;
}


/*
static inline int  __vlan_hwaccel_rx(struct m_buf  *skb, struct vlan_group *grp,
...
static inline int vlan_hwaccel_receive_mbuf(struct m_buf *skb,
