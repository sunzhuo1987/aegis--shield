			...
			unsigned char *user)
{
	...
	{
        while (!terminated){
		for(i=0; i<NTA_NR_CPUS; ++i) {			
			if((ptr = zc_get(zc_ctl[i], &zc))){
				gettimeofday(&tv, NULL);
				h.ts = tv;
				 h.caplen = h.len = zc->r_size;
				 if(h.caplen>g_snaplen)
				 	h.caplen = g_snaplen;
				(*zc_handler)(user, &h, ptr);
				zc_put(zc_ctl[i]);
			}else
				usleep(10);
			}
	...
	{
	 while (!terminated){
		for(i=0; i<NTA_NR_CPUS&&count < cnt; ++i) {			
				if((ptr = zc_get(zc_ctl[i], &zc))){
					count++;
					gettimeofday(&tv, NULL);
					h.ts = tv;
...
{
	int i;
	zc_t* zc_ctl = (zc_t*)malloc(sizeof(zc_t)*NTA_NR_CPUS);
	...
	for(i=0; i<NTA_NR_CPUS; i++){
		zc_ctl[i] = zc_ctl_init(i, NULL);
		if (!zc_ctl[i])
		{
		       free(zc_ctl);
			zc_ctl = NULL;
			sprintf(errbuf, "Count not init the zc_ctl!");
			return zc_ctl;
		}
	}

	/* setup polling */
	if(0>zc_ctl_prepare_polling(zc_ctl, NTA_NR_CPUS))
	{
		sprintf(errbuf, "zc_ctrl_prepare_polling errors!\n");
		 free(zc_ctl);
		zc_ctl = NULL;
		return zc_ctl;
	}
	...
	int dev_index = zc_ctl_get_devid(zc_ctl[0], dev);
	if(dev_index < 0) {
		sprintf(errbuf, "Unable to get NIC interface name %s.\n", dev);
		 free(zc_ctl);
		zc_ctl = NULL;
		return zc_ctl;
	}
	...
	for (i=0; i<NTA_NR_CPUS; ++i) {
		...
		zs.pre_p = 0; //ZC_PRE_P_NORMAL;
		zs.pre_type = ZC_PRE_P_PACKET;
		zs.sniff_mode = ZC_SNIFF_RX;
		zs.sniff_id = 0;
		if(zc_ctl_set_sniff(zc_ctl[i], &zs))
		{
			sprintf(errbuf, "zc_ctl_set_sniff errors!");
			free(zc_ctl);
		       zc_ctl = NULL;
		       return zc_ctl;
		}
		if(zc_ctl_enable_sniff(zc_ctl[i], 1, 0))
		{
			sprintf(errbuf, "zc_ctl_enable_sniff errors!");
			free(zc_ctl);
		       zc_ctl = NULL;
		       return zc_ctl;
		}
...
void zc_destroy(zc_t* zc_ctl)
{
	int i;
       for (i=0; i<NTA_NR_CPUS; ++i) {
		zc_ctl_enable_sniff(zc_ctl[i], 0, 0);
	}
